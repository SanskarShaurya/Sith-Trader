...
public:
    ...
    Package operator*(int rhs){
        ...
        for(auto i: temp) temp.search(i.first) = i.second * rhs * isBuy;
        return Package(temp,1,this->price*rhs*isBuy, this->indexes, this->quantity);
    }
    ...
    void printPackage(){
        for(auto i : stocks){
            if(i.second!=0) std::cout << i.first.name << " " << i.second << " ";
        }
        std::cout << price << " ";
    ...
    void set_max(RedBlackTree<myPair<int>> maxSum)
    {
        for (auto k : this->stocks)
        {
            ...
            auto c = this->stocks.search(k.first)*isBuy;
            if(c==0) continue;
